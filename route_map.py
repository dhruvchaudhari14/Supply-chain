...
if tab == "Optimal Solution without capacity constraints.":
    ...
    if run_code:
        ...
        st.download_button('Download The Network Map', results, 'Iteration_1_network_map.csv', 'csv/txt')
        st.download_button('Download the additional data',df,'Iteration_1_additional_data.csv','csv/txt')
...
#                 'Markets Fulfilled': int(num_markets_fulfilled)
        ...
        df_3 = df_3.to_csv(index = False).encode("utf-8")
        st.write("Click to download results from the above iteration in csv format")
